...
var vschess={
    ...
    soundlist:"n1 n2 n3 n4 n5 n6 n7 n8 n9 click bomb eat move check lose illegal r n b a c rk bk rp bp jin tui ping qian zhong hou".split(" ")
};
    $.each(("play time step result automove autoMove time_each sound soundState turn move clip format usermove setsteps oldcontent "+"tips pfchess pfreload livepause pf2str senddom saveqq saveqqm currentfen beginfen situation resetdom putchessman "+"checkmove controlbar eachstep moveindex comments eachposition animate getnewdom chinesestep chinesestepm chinesepgn "+"chinesepgnm chinesehtml chinesehtmlm wxfstep wxfstepm wxfpgn wxfpgnm wxfhtml wxfhtmlm iccsstep iccsstepm iccspgn "+"iccspgnm iccshtml iccsmhtml chineseSoundStatus volume").split(" "),
      function(){
    ...
    vschess.turnfen=function($str){var $i=0,$j=0;var $array=$str.split(" ");var $old=$array[0];var $new=[],$oldline=[];$old=$old.split("/");for($i=0;$i<10;++$i){$oldline=$old[$i].split("");
    for($j=$oldline.length-1;$j>=0;--$j){$new.push($oldline[$j]);
}
if($i!=9){$new.push("/");}
}
